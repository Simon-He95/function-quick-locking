...
export function parserJavascript(ast: VueAst) {
  ...
  body.forEach((item: any) => {
    ...
    if (type === 'FunctionDeclaration') {
      ...
      if (item.declaration?.declarations?.[0]?.init?.type === 'ArrowFunctionExpression' || item.declaration.declarations?.[0]?.init?.type === 'FunctionExpression' || item.declaration.type === 'FunctionDeclaration')
        methods.push(item)
